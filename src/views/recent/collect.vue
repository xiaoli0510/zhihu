<template>
    <div class="collect-page-top">
        ...
        <van-row>
            <van-col>{{ refCurBookmark.bookmarkList[0].folderTitle }}</van-col>
            <van-col v-if="refCurBookmark.bookmarkList[0].power === 1"><van-icon name="bag" /></van-col>
        </van-row>
        <van-text-ellipsis :content="refCurBookmark.bookmarkList[0].depict" />
    </div>
    <div class="collect-page-main">
        <p class="explain">{{ refCurBookmark.bookmarkList[0].childrenNum }}个内容</p>
        <CollectItem @share="onShare" v-for="(item, index) in refCurBookmark.collectList" :key="item.id" :item="item"
...
watch(list, (newValue) => {
    refCurBookmark.value.bookmarkList= newValue.bookmarkList.filter(item => item.id === 2)
    refCurBookmark.value.collectList= newValue.collectList.filter(item => item.parentId.includes(2))
    lastBookmarkId.value = newValue.bookmarkList[newValue.bookmarkList.length - 1].id
},{
    immediate:true,
    deep:true,
})
